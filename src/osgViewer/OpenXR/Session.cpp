    ...
    _localSpace(XR_NULL_HANDLE)
{
    ...
    size_t bindingLength = window->getXrGraphicsBinding(nullptr);
    if (!bindingLength)
    {
    ...
    uint8_t *graphicsBinding = new uint8_t[bindingLength];
    bindingLength = window->getXrGraphicsBinding(graphicsBinding);
